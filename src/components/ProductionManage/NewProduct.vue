...
<script>
  ...
  import {reqAddProduct} from '../../api/product'
  ...
  export default {
    ...
    data(){
      return{
        ...
        rules:{
          productNum:[
            {type: 'string',required:true,message:'请填写产品编号',trigger:'blur'}
          ],
          productName:[
            {type: 'string',required:true,message:'请填写产品名称',trigger:'blur'}
          ]
        },
        loading:false
      }
    ...
    methods:{
      ...
      modalClose(msg){
        if(msg===1){
          PubSub.publish('product-dialog-close',1)
        }
        else{
